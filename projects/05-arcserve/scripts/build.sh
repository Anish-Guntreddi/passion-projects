#!/usr/bin/env bash
# Configures and builds ArcServe. Usage:
...
case "${SANITIZER}" in
  asan)
    CMAKE_ARGS+=(-DARCSERVE_ENABLE_ASAN=ON -DARCSERVE_ENABLE_UBSAN=ON)
    ;;
  ubsan)
    CMAKE_ARGS+=(-DARCSERVE_ENABLE_UBSAN=ON)
    ;;
  tsan)
    CMAKE_ARGS+=(-DARCSERVE_ENABLE_TSAN=ON)
    ;;
  none)
    ;;
  *)
