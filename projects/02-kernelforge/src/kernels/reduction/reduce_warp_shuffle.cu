...
__global__ void reduce_warp_shuffle_kernel(const float* __restrict__ in, float* __restrict__ out,
                                            std::size_t n) {
  ...
  // Sequential addressing down to exactly one warp's worth of partials
  // (identical to V2 for these steps).
  ...
  // ONE variable changed vs V2: the last warp finishes with warp-shuffle
  // intrinsics instead of 5 more shared-memory + __syncthreads() rounds.
  // ADR 0004: `_sync` family, full mask -- every thread reaching this
  // branch (tid < 32) is warp-uniform.
  if (tid < 32) {
    float val = sdata[tid];
...
void reduce_warp_shuffle_launch(const float* d_in, float* d_out, std::size_t n, int block_size,
                                 ...
                                 bool zero_output) {
  validate_block_size_pow2_1d(block_size, "reduce_warp_shuffle_launch");
  if (zero_output) {
  ...
  reduce_warp_shuffle_kernel<<<grid, block, smem_bytes, stream>>>(d_in, d_out, n);
...
float reduce_warp_shuffle_run_host(const float* h_in, std::size_t n, float* h_sum_out,
  ...
  dim3 grid, block;
  timer.start();
  reduce_warp_shuffle_launch(d_in, d_out, n, block_size, 0, grid, block);
